...
# NEW: 不同类别（含 obs_ 前缀）使用不同基础半径
PER_CLASS_RADIUS = {
    "aruco": 0.04,

    # 动态障碍（yolo_obs_logger 写入）
    "obs_orange": 0.03,
    "obs_lemon": 0.035,
    "obs_pear": 0.03,
    "obs_tomato": 0.04,
    ...
    # 地图里原有的静态目标（若作为障碍）
    "orange": 0.03, "lemon": 0.035, "pear": 0.03, "tomato": 0.040,
    "capsicum": 0.045, "potato": 0.045, "pumpkin": 0.045, "garlic": 0.03,
}

def _radius_for_name(name, default_r):
    n = (name or "").lower()
    best_key, best_r = None, None
    for k, r in PER_CLASS_RADIUS.items():
        if k in n:
            if best_key is None or len(k) > len(best_key):
                best_key, best_r = k, r
    return best_r if best_r is not None else default_r
...
class SafeLOSPlanner:
    ...
    def _r_keep_for(self, name, target_name, prev_target_name):
        ...
        bare  = _radius_for_name(name, self.cfg["obstacle_radius"])
    ...
    def _segment_clear(self, x1, y1, x2, y2, tube, ignore_names=None):
        ...
        # 计算“额外余量”：调用方可能传入 self.los_tube + extra_clearance
        base_tube = self.cfg["robot_radius"] + self.cfg["obstacle_radius"] + self.cfg["inflate_error"]
        tube_extra = max(0.0, tube - base_tube)
        ...
        for (ox, oy, name) in self.obstacles:
            ...
            # —— 关键：按障碍名称取半径，并叠加“额外余量”
            r_obs  = _radius_for_name(name, self.cfg["obstacle_radius"])
            tube_i = self.cfg["robot_radius"] + r_obs + self.cfg["inflate_error"] + tube_extra

            if math.hypot(ox - cx, oy - cy) <= tube_i:
                return False
        return True
    ...
    def plot_run(self, rx, ry, obstacles, sx, sy, gx, gy, target_name=None, prev_target_name=None):
        ...
        for (x, y, name) in obstacles:
            ...
            r_obs = _radius_for_name(name, cfg["obstacle_radius"])
            ax.add_patch(plt.Circle((px, py), r_obs, color=color, alpha=0.3))

            # 膨胀圈
            ax.add_patch(plt.Circle((px, py), r_obs + cfg["inflate_error"], fill=False, linestyle="--",

...
class EKF:
    ...
    Measurement noise tuned to match empirical ArUco data:
    - <0.5 m → ~1–2 cm
    - 0.5–1.4 m → ~2–4 cm
    - >1.4 m → up to 10–15 cm (jitter), handled via +15 cm inflation
    """

    # ===================== TUNABLES =====================
    BASE_MEAS_STD_M   = 0.02     # ~2 cm baseline
    ...
    INIT_LM_STD_M     = 0.40     # large init std for new LM (safety margin)
    USE_SMART_LM_INIT = True

    # Anchor landmark (first tag)
    ANCHOR_USE        = True
    ANCHOR_LM_STD_M   = 0.02     # 2 cm init covariance
    ANCHOR_MEAS_STD_M = 0.03     # 3 cm measurement std

    # Range management
    MAX_RANGE_M   = 1.4     # only add new landmarks if ≤ 1.4 m
    FAR_REJECT    = False   # keep updating far landmarks
    FAR_EXTRA_STD = 0.15    # +15 cm error inflation if beyond 1.4 m
    ...
    def __init__(self, robot):
        ...
        if self.OPTIONAL_ALIGNMENT:
            ...
            self.eval_R = None
            self.eval_t = None
    def preload_map(self, map_dict):
        """
        Preload all landmarks (aruco + fruits) from a given map dictionary.
        map_dict: { "name": {"x": float, "y": float}, ... }
        """
        self.markers = np.zeros((2,0))
        self.taglist = []
        self.P = np.zeros((3,3))   # reset covariance (robot only)

        for i, (name, pos) in enumerate(map_dict.items()):
            x, y = float(pos["x"]), float(pos["y"])
            self.markers = np.hstack((self.markers, np.array([[x],[y]])))
            self.taglist.append(name)

            # expand covariance matrix with small prior uncertainty
            oldP = self.P
            self.P = np.zeros((oldP.shape[0]+2, oldP.shape[1]+2))
            self.P[:oldP.shape[0], :oldP.shape[1]] = oldP
            s2 = (0.05**2)   # ~5 cm prior std
            self.P[-2:, -2:] = np.diag([s2, s2])

        print(f"[EKF] Preloaded {len(self.taglist)} landmarks from true map")
    ...
    def _adaptive_R(self, z_i, d2=None, is_anchor=False):
        """
        Measurement covariance model:
        - Anchor: trust more tightly
        - <0.5 m: ~2 cm
        - 0.5–1.4 m: gentle inflation
        - >1.4 m: add +15 cm error inflation
        """
        if is_anchor:
        ...
        if r < 0.5:
            # very close: keep tight (baseline only)
            R = Rb
        elif r <= self.MAX_RANGE_M:
            # mid-range: inflate softly
            ...
            # far beyond tested range: add flat +15 cm inflation
    ...
    def update(self, measurements):
        ...
        kept_rows, R_adapt = [], np.zeros_like(R_stack)
        for i, tag in enumerate(tags):
            ...
            R_i_ad = self._adaptive_R(z_i, d2=d2, is_anchor=is_anchor)
    ...
    def add_landmarks(self, measurements):
        ...
        for lm in measurements:
            ...
            lm_bff = lm.position
            r = np.linalg.norm(lm_bff)

            # only add if within reliable tested range
